...
const server = new McpServer(
  ...
  },
  {
    instructions:
      ...
      "COLLECTION IS THE USER'S: Auralink ships no headphone database. Every profile and curated preset lives in the user's own collection directory, so an empty headphone list on a fresh install is expected, not a defect — look the model up with get_autoeq_correction instead of assuming data should already be there. " +
      "When adding a new headphone/earphone model, call register_headphone_baseline (AutoEq name, or bands+type when AutoEq misses). It writes the profile and its baseline into the user's collection — do not create scripts/, do not commit, and do not use that tool for Luxsin X8. For later preference tuning, audition first with audition_eq_preset and save only when the user likes it or asks to save. Nothing else enters the collection automatically: call add_preset_to_collection only when the user asks to keep or share a preset. " +
